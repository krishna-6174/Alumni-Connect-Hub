      ...
      console.log(data);
        try {
            ...
            if (response.data.goAhead) {
                ...
                sessionStorage.setItem("user_avatar",response.data.user.profile);
                authContext?.setUser({
                ...
                setTimeout(() => {
                    navigate(response.data.user.role==="alumni" ? "/alumni/dashboard" : "/admin/dashboard"); // Redirect based on role
                }, 1000);
            } else {
                loading?.showLoading(false);

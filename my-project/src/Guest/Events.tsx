...
const Events = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        const response = await axios.get('/api/getevents', { withCredentials: true });
              ...
              {eventCategories.map((category) => (
                <li
                  ...
                  className={`cursor-pointer p-2 rounded-lg ${
                    activeCategory === category.category ? 'bg-indigo-400' : 'bg-gray-200'
                  } hover:bg-indigo-300`}
              ...
              <Grid container spacing={4}>
                {filteredEvents.length > 0 ? (
                  filteredEvents.map((event) => (
                    <Grid item key={event.event_id} xs={12} sm={12} md={12}>
